...
async function init() {
    ...
    if (urlFilter) {
        // Update Title
        const titleEl = document.getElementById('women-title');
        let formattedTitle = urlFilter.charAt(0).toUpperCase() + urlFilter.slice(1);
        ...
        document.title = `${formattedTitle} — GANPATIS`;
    }
    ...
    try {
        ...
        allProducts = data.filter(p => p.tags && p.tags.some(t => 
            t.toLowerCase().includes('women') || 
            t.toLowerCase().includes('womenswear') || 
